...
app.get('/readfile', (req, res) => {
  let name = req.query.name || 'README.md';
   
  const filenameRegex = /^[a-zA-Z0-9]+$/;

    if (!filenameRegex.test(name)) {
      return res.status(401).send('nom de fichier invalide');
    }

    name = `${req.query.name}.sarif`;
...
app.post('/ping', (req, res) => {
  try {
    ...
    if (!ipRegex.test(host)) {
      return res.status(401).send('Adresse IP non autorisée');
    }
